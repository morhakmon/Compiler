...
L_code_ptr_bin_apply:
      ...
      mov rbx, qword[rsp + 8 * 4]         ; rbx = list s 
      mov rdi, 0                          ; rdi = counter size of the list
      mov rdx, rsp                        

      .push_list:                         ; we push the list from start to end , need to switch
        cmp rbx, sob_nil
        je .exit_push_list
        add rbx, 1                        ; go to  car, not to tag pair
        push qword[rbx]
        inc rdi
        mov rbx, qword[rbx + 8]
        jmp .push_list
      ...
      .do_switch:
        ...
        mov r12, rcx
        IMUL r12, (-1)
        mov rbx, qword[rdx + 8 * r12]
        mov r12, rdi
        IMUL r12, (-1)
        mov rax, qword[rdx + 8 * r12]
        mov r12, rdi
        IMUL r12, (-1)
        mov qword[rdx + 8 * r12], rbx
        mov r12, rcx
        IMUL r12, (-1)
        mov qword[rdx + 8 * r12], rax
        inc rcx
        dec rdi
        jmp .do_switch
      ...
      .exit_do_switch:
       ...
       mov r15, qword[rdx + 8 * 3]     ; r15 = proc 
       push qword[r15 + 1]        ; push lexenv of proc (for example +)
       push qword[rdx]            ; push ret          
       

    mov rdx, [rsp + 8 * 2]       
    mov rsi, rdx                  ; rsi = size of list 
    add rsi, 2                    ; rsi =  how much we need to add to rsp to get to top of blue
    mov rdi, rsi
    add rdi, 5                    ; rdi = size of blue + 5 (how much we need to add to rsp to get to top of green)


    mov rax, 8          ; rax = qword_size
    mul rdi
    mov rdi, rax 
    add rdi, rsp        ; rdi = address of top stack of g (list s) 
    ...
    mov rsp, rdi
    mov r15, qword[r15 + 9] ; r15 = proc code
    jmp r15
