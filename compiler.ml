...
module Reader : READER = struct
  ...
  and nt_paired_comment str = (*hw1*)
  let ntOpen = unitify (char '{') in
  let ntClose = unitify (char '}') in
  let nt1 = unitify (diff nt_any (disj ntOpen ntClose)) in
  let nt2 = unitify (star (disj (unitify (word "#\\{")) 
                          (disj (unitify (word "#\\}"))     
                          (disj (unitify nt_string) nt1)))) in
  let nt3 = unitify (caten_list[ntOpen;nt2;unitify(star nt_paired_comment);nt2;ntClose]) in
  nt3 str


  and nt_sexpr_comment str = (*hw1*)
    let nt1 = word "#;" in
    let nt2 = caten nt1 nt_sexpr in
    let nt1 = unitify nt2 in
    nt1 str
  ...
  and make_skipped_star (nt : 'a parser) =
    ...
    nt1
  and nt_digit str =(*hw1*)
    let nt1 = range '0' '9' in
    let nt1 = pack nt1 (fun d -> (int_of_char d) - (int_of_char '0')) in
    nt1 str 
  
  and nt_hex_digit str = (*hw1*)
    let ntLower = range 'a' 'f' in
    let ntLower = pack ntLower (let delta = int_of_char 'a' - 10 in
                    fun d -> int_of_char d - delta) in
    let ntUpper = range 'A' 'F' in
    let ntUpper = pack ntUpper (let delta = int_of_char 'A' - 10 in
                    fun d -> int_of_char d - delta) in
    let nt1 = disj_list [ntLower; ntUpper; nt_digit] in
    nt1 str
  ...
  and nt_nat str = (*hw1*)
    ...
    let nt1 = pack nt1 
                (fun digits -> 
                  List.fold_left 
                    (fun number nextDigit -> 10 * number + nextDigit)
                  0
                  digits) in
    nt1 str
    ...
    let nt1 = pack nt1
                (fun digits ->
                  List.fold_left
                    ...
                    digits) in
    nt1 str
  and nt_optional_sign str = (*hw1*)
    let ntNeg = pack (char '-') (fun _ -> false) in
    let ntPos = pack (char '+') (fun _ -> true) in
    let nt1 = maybe (disj ntNeg ntPos) in
  ...
  and nt_float str = (*hw1*)
    let nt1 = nt_optional_sign in
    let nt_FloatA = caten nt_integer_part
                      (caten (char '.')
                        (caten (make_maybe nt_mantissa 0.0)
                        (make_maybe nt_exponent 1.0))) in
    let nt_FloatA = pack nt_FloatA (fun (ip, (_, (mant,mult))) ->
                            (ip +. mant) *. mult) in

    let nt_FloatB = caten (char '.')
                      (caten nt_mantissa (make_maybe nt_exponent 1.0)) in
    let nt_FloatB = pack nt_FloatB (fun (_, (mant, mult)) -> 
                            mant *. mult) in

    let nt_FloatC = caten nt_integer_part nt_exponent in
    let nt_FloatC = pack nt_FloatC (fun (ip, mult) -> 
                            ip *. mult) in

    let nt_Float = disj nt_FloatA (disj nt_FloatB nt_FloatC) in
    let nt1 = pack (caten nt1 nt_Float) (fun (is_positive, x) -> 
  ...
  and nt_char_named str = (*hw1*) 
  let nt1 = (fun name ch -> pack (word_ci name) (fun _ -> ch)) in
  let nt2 = disj_list [(nt1 "newline" '\n');
                        (nt1 "nul" (Char.chr 0));
                        (nt1 "page" '\012');
                        (nt1 "return" '\r');
                        (nt1 "space" ' ');
...
and nt_string_part_simple str =
  ...
  and nt_string_part_hex str =
    ...
    nt1 str
  and nt_string_part_dynamic str = (*hw1*)
  ...
  and nt_string str =
    ...
    nt1 str
  and nt_vector str = (*hw1*)
    let ntOpen = word "#(" in
    let nt1 = caten ntOpen nt_skip_star in
    let ntClose = char ')' in
    let nt2 = pack ntClose (fun _ -> ScmVector []) in
    let nt3 = plus nt_sexpr in
    let nt3 = caten nt3 ntClose in
  ...
  and nt_list str = (*hw1*)
    let ntOpen = char '(' in
    let ntClose = char ')' in
    let nt1 = pack (caten nt_skip_star ntClose) (fun _ -> ScmNil) in
    let ntProper = pack ntClose (fun _ -> ScmNil) in
    let ntImproper = pack (caten (char '.') (caten nt_sexpr ntClose))
                                          (fun ( _, (sexpr,_)) -> sexpr) in
    let nt2 = disj ntProper ntImproper in
    let nt3 = pack (caten (plus nt_sexpr) nt2) (fun (sexprs, sexpr) -> 
                                        List.fold_right
                                         (fun car cdr -> ScmPair (car, cdr))
                                        sexprs
                                        sexpr) in

    let nt3 = disj nt1 nt3 in
    let nt4 = pack (caten ntOpen nt3) (fun ( _,sexpr) -> sexpr) in
    make_skipped_star nt4 str 
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec proper_scheme_list_to_list = function
    | ScmPair (car, cdr) -> car :: (proper_scheme_list_to_list cdr)
    | ScmNil -> []
    | sexpr -> raise (X_syntax "Not a proper list")
  ;;

  let rec list_to_proper_scheme_list = function
    | [] -> ScmNil
    | car :: [] -> ScmPair (car, ScmNil)
    | car :: cdr -> ScmPair (car, list_to_proper_scheme_list cdr)
  ;;
  ...
  let create_list_of_cars list_of_lists =
    (List.map get_car list_of_lists);;


  let create_list_of_cdrs list_of_lists = 
    (List.map get_cdr list_of_lists);;

  let proper_list_of_cars sexpr = 
     (list_to_proper_scheme_list (create_list_of_cars (proper_scheme_list_to_list sexpr)));;
  
  let proper_list_of_cdrs sexpr = 
     (list_to_proper_scheme_list (create_list_of_cdrs (proper_scheme_list_to_list sexpr)));;
  ...
  let rec scheme_map f sexpr =
    match sexpr with
      ...
      | _ -> raise (X_syntax "Not a proper list")
  ;;

  let add_setbang sexp1 sexp2 = 
    ScmPair(ScmSymbol "set!", ScmPair(sexp1, ScmPair(sexp2, ScmNil)));;

  let add_quote_whatever sexp = 
    ScmPair(sexp, ScmPair(ScmPair(ScmSymbol "quote", 
      ScmPair(ScmSymbol "whatever", ScmNil)), ScmNil));;
  
  
  let rec combine_lists_with_func func sexpr1 sexpr2 =
    match sexpr1, sexpr2 with
      ...
      | _, _ ->
          raise (X_syntax "Expects two lists of equal length")
  ;;
  ...
  let rec macro_expand_and_clauses expr = function
    ...
    | (expr' :: exprs) as es-> 
        ...
        ScmPair(expr, 
        ScmPair(ScmPair(ScmSymbol("and"),list_to_proper_scheme_list es),
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
       (**) tag_parse (ScmPair(ScmPair (ScmSymbol "lambda", ScmPair
       (list_to_proper_scheme_list (create_list_of_cars (proper_scheme_list_to_list ribs)),
       exprs)),
       list_to_proper_scheme_list (create_list_of_cdrs (proper_scheme_list_to_list ribs))))
  ...
  (**)(match ribs with 
        ...
        |_ -> tag_parse (append_exprs_to_list 
                          (ScmPair (ScmSymbol "let", ScmPair(
                            (scheme_map add_quote_whatever (proper_list_of_cars ribs)),
                            combine_lists_with_func add_setbang (proper_list_of_cars ribs) 
                            (proper_list_of_cdrs ribs)))) 
                           exprs))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let should_box_var name expr params = 
    let (s1, s2) = find_reads_and_writes name expr params in
      match (s1, s2) with 
        | ([], []) -> false
        | ([], _) -> false
        | (_, []) -> false
        | (_, _) -> 
          ...
          let list_of_read_write = cross_product s1 s2 in
          let rec find_rib name = function
            | [] -> raise (X_this_should_not_happen "empty env")
            | rib :: env -> if (List.mem name rib) 
                              then (rib : string list)
                              else find_rib name env in
          
          let different_env = (List.filter(fun ((var1, env1), (var2, env2)) ->
            match ((var1, env1), (var2, env2)) with                            
              | ((Var'(_, Bound(_,_)), _),(Var'(_, Param(_)), _)) -> true                  
              | ((Var'(_, Param(_)), _), (Var'(_, Bound(_,_)), _)) -> true
              | ((Var'(_, Bound(_,_)), env1),(Var'(_, Bound(_,_)), env2)) -> 
                  (not ((find_rib name env1) == (find_rib name env2)))
              | _ -> false)
            list_of_read_write) in 


          match different_env with
            |[] -> false
            | _ -> true
        ;;        
...
module Code_Generation (*: CODE_GENERATION*)= struct
  ...
  let remove_duplicates (**)const_list = 
    let rec build_set acc remain =
      match remain with 
      | [] -> acc
      | head :: tail ->
        if (List.mem head acc) 
          then (build_set acc tail) 
          else (build_set (head :: acc) tail) in
      
    List.rev(build_set [] const_list)    
  ;;
  ...
  let add_sub_constants =
    ...
    in fun exprs' ->
      ...
      match exprs' with
        | [] -> []
        | expr' :: exprs' -> List.flatten(List.map run ([expr'] @ exprs'))
    ;;


  let search_constant_address constant table = 
    let rec search constant table= 
      match table with
      | [] -> raise (X_this_should_not_happen "empty const table")
      | (constant', loc, rep) :: rest -> if (constant = constant') then loc 
                                                              else (search constant rest) 
      in search constant table ;;  
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmBox' (Var' (v, Param minor)) ->  
        ...
        call malloc
        mov rbx, qword [rbp + 8 * (4 + %d)]
        mov qword [rax], rbx\n" minor) 

      | ScmBox' _ -> raise (X_this_should_not_happen "not param")
      | ScmBoxGet' var' ->
      ...
      | ScmLambda' (params', Opt opt, body) -> 
          ...
          and label_end_opt = make_lambda_opt_end ()
          and label_lambda_opt_stack_ok = make_lambda_opt_stack_ok()
          ...
          ^ (Printf.sprintf "%s:\n" label_arity_more) 
          ^ (Printf.sprintf "\tMORE_PARAM (%d)\n" (List.length params'))
          ^ (Printf.sprintf "\tjmp %s\n" label_lambda_opt_stack_ok)
          
          ^ (Printf.sprintf "%s:\n" label_arity_exact)
          ^ (Printf.sprintf "\tEXACT_PARAM (%d)\n" (List.length params'))
